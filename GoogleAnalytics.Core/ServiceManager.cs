...
namespace GoogleAnalytics
{
    ...
    public class ServiceManager : IServiceManager
    {
        ...
        static readonly Uri endPointSecureGA4 = new Uri("https://analytics.google.com/g/collect");
        ...
        async Task<HttpResponseMessage> SendHitAsync(Hit hit, HttpClient httpClient, IDictionary<string, string> hitData)
        {
            if (IsGA4)
            {
                return await httpClient.GetAsync(endPointSecureGA4 + "?" + GetUrlEncodedString(hitData)); // Postメソッドだとなぜか計測してくれないのでGetメソッドを使用。
            }
            else
            {
