...
namespace Redmine.Net.Api
{
    ...
    public class RedmineManager : IRedmineManager
    {
        ...
        // カスタマイズ(S):「ユーザ名/パスワード」をBasic認証のために使用し、「APIキー」で Redmine にアクセスする対応
        /// <summary>
        /// ユーザ名/パスワードとAPIキーを両方指定するコンストラクタ
        /// </summary>
        ...
        // カスタマイズ(E):「ユーザ名/パスワード」をBasic認証のために使用し、「APIキー」で Redmine にアクセスする対応
        ...
        public List<WikiPage> GetAllWikiPages(string projectId)
        {
            ...
            // カスタマイズ(S):Wiki ページがプロジェクトに設定されていない場合 result.Items が null になるので null チェック追加
            return result == null || result.Items == null ? new List<WikiPage>() : new List<WikiPage>(result.Items);
            //return result == null ? null : new List<WikiPage>(result.Items);
            // カスタマイズ(E):Wiki ページがプロジェクトに設定されていない場合 result.Items が null になるので null チェック追加
        }
        ...
        public virtual RedmineWebClient CreateWebClient(NameValueCollection parameters, bool uploadFile = false)
        {
            ...
            if (!string.IsNullOrEmpty(ApiKey))
            {
                ...
                // カスタマイズ(S):「ユーザ名/パスワード」をBasic認証のために使用し、「APIキー」で Redmine にアクセスする対応
                // もともと API キーが指定されていた場合、この処理は無かった
                if (cache != null)
                {
