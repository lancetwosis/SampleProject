...
namespace Redmine.Net.Api.Types
{
    ...
    public sealed class IssueCustomField : IdentifiableName, IEquatable<IssueCustomField>, ICloneable, IValue
    {
        ...
        public override void ReadXml(XmlReader reader)
        {
            ...
            // Project の issue_custom_fields の場合、以下のような形で取得される
            // <issue_custom_fields type="array">
            //   <custom_field id="7" name="必須レビューア"/>
            //   <custom_field id="8" name="対面レビュー"/>
            //   <custom_field id="9" name="検出工程"/>
            //   <custom_field id="11" name="指摘者"/>
            // </issue_custom_fields>
            // 従来の処理は以下のように <value /> が存在していることを期待した処理となっていた。
            //   <custom_field id = "7" name = "必須レビューア" >
            //     <value />
            //   </custom_field>
            // よって、次の Element が ID や NAME を持っていた場合、Value を取得する処理をスキップするように修正した。
            var id = reader.GetAttribute(RedmineKeys.ID);
            var name = reader.GetAttribute(RedmineKeys.NAME);
            if (reader.NodeType == XmlNodeType.EndElement || (!string.IsNullOrEmpty(id) && !string.IsNullOrEmpty(name)))
                return;
            // カスタマイズ(E):Project の CustomFields を取得するために実施したカスタマイズ
